...
describe('locateProject', () => {
  test('returns the project folder based on Cargo.toml', async () => {
    const mockCargo = spawk.spawn('cargo').stdout(`${cwd}/Cargo.toml`);
...
describe('generate', () => {
  test('runs cargo-generate', async () => {
    ...
    expect(mockCargo.calledWith).toMatchObject({
      args: ['generate', '--name', name, '--git', repository, '--branch', branch, template],
...
describe('metadata', () => {
  test('returns the project JSON metadata', async () => {
    const mockCargo = spawk.spawn('cargo', arrayStartsWith('metadata')).stdout(JSON.stringify(Fixtures.singleProject));
    ...
    expect(mockCargo.calledWith).toMatchObject({
      args: ['metadata', '--quiet', '--no-deps', '--format-version=1'],
...
describe('projectMetadata', () => {
  test('parses the medata for single project definition', async () => {
    spawk.spawn('cargo', arrayStartsWith('metadata')).stdout(JSON.stringify(Fixtures.singleProject));
  ...
  test('parses the metadata of the first project in workspace project definitions', async () => {
    spawk.spawn('cargo', arrayStartsWith('metadata')).stdout(JSON.stringify(Fixtures.workspaceProject));
  ...
  test('parses the metadata for the project in current dir', async () => {
    ...
    spawk.spawn('cargo', arrayStartsWith('locate-project')).stdout(`${cwd}/contracts/bar/Cargo.toml`);
  ...
  test('fails if could not resolve project metadata', async () => {
    ...
    spawk.spawn('cargo', arrayStartsWith('locate-project')).stdout(`${cwd}/contracts/baz/Cargo.toml`);
