...
async function executeTx(archwayd, options) {
  ...
  const { txhash } = await archwayd.tx.wasm('execute', [contract, args], txOptions);

  console.info(chalk`{green Executed tx on contract {cyan ${contract}} with tx hash {cyan ${txhash}}}\n`);
}
