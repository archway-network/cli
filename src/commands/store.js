...
async function verifyUploadedWasm(
  archwayd,
  config,
  { project: { name: projectName, wasm: { optimizedFilePath } = {}, workspaceRoot } = {}, chainId, node } = {}
) {
  ...
  await retry(() => archwayd.query.wasmCode(codeId, localDownloadPath, { node }), {
  ...
  if (localChecksum !== onChainChecksum) {
    throw new Error(
      `Local wasm file does not match on-chain checksum local=${localChecksum} on-chain=${onChainChecksum}`
    );
  }
